...
class BrandearScraper(PlaywrightScraper):
    ...
    def search(self, brand: str) -> list[Listing]:
        ...
        try:
            ...
            cards = self.parse_cards(html, ["li.productList_item", "div.productBox", "a[href*='/search/detail/']"])[: self.config.max_items]
            listings: list[Listing] = []
            for card in cards:
                title = self.pick_text(card, ["[class*='product_name']", "[class*='itemName']", "h3", "a"])
