...
class TrefacFashionScraper(PlaywrightScraper):
    ...
    base_url = "https://www.trefac.jp/store/search"

    def __init__(self, config: ScraperConfig) -> None:
        super().__init__(config)

    def build_search_url(self, brand: str) -> str:
        return f"{self.base_url}?word={encode_query(brand)}"

    def search(self, brand: str) -> list[Listing]:
        self.begin_search_stats(brand)
        try:
            html = self.fetch_html(self.build_search_url(brand))
            cards = self.parse_cards(html, ["li.search-list-item", "div.itemBox", "a[href*='/smartphone/']"])[: self.config.max_items]
            listings: list[Listing] = []
            for card in cards:
                title = self.pick_text(card, ["[class*='item-name']", "[class*='itemName']", "h3", "a"])
                price_text = self.pick_text(card, ["[class*='price']", "span"])
                url = self.pick_attr(card, ["a[href*='/smartphone/']", "a[href]"], "href")
                if url.startswith("/"):
                    url = f"https://www.trefac.jp{url}"
